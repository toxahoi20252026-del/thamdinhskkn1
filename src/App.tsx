...
export default function App() {
    ...
    const fetchData = async (retries = 3) => {
      try {
        ...
        const [initiativesRes, settingsRes, healthRes] = await Promise.all([
          fetchWithRetry('/api/initiatives').catch(() => ({ ok: false } as Response)),
          fetchWithRetry('/api/settings').catch(() => ({ ok: false } as Response)),
          fetchWithRetry('/api/health').catch(() => ({ ok: false } as Response))
        ]);
        ...
        if (savedUser) {
          ...
          // Keep default user if no session
          setCurrentUser({
            id: 'admin-1',
            username: 'admin',
            role: 'admin',
  ...
  const handleLogout = () => {
    // Instead of null, reset to default user to bypass login
    ...
    docElements.push(new Paragraph({
      children: [
        new TextRun({ 
          ...
          bold: true, 
          size: 48, 
          color: target.score >= 8 ? "228B22" : target.score >= 5 ? "FF8C00" : "FF0000" 
        })
      ],
      alignment: AlignmentType.CENTER,
      spacing: { after: 400 }
